...
export async function GET(req: Request) {
  ...
  try {
    const user = await client.fetch(`*[_type == "user" && confirmationToken == $token][0]`, { token: token })

    if (!user) {
      return NextResponse.json({ message: "User not found" }, { status: 404 })
    }

    await client.patch(user._id).set({ isConfirmed: true, confirmationToken: null }).commit()
